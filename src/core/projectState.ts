...
export function parseProjectSnapshot(raw: string): ProjectSnapshot {
  ...
  for (const building of value.buildings) {
    ...
    buildings.push({
      ...building,
      function: functionForPlacedBuilding(building),
    });
  }
...
function isPlacedBuilding(value: unknown): value is PlacedBuilding {
  return (
    ...
    (value.function === undefined ||
      ...
      value.function === "retail" ||
      value.function === "office" ||
      value.function === "university" ||
      value.function === "library" ||
      value.function === "school" ||
      value.function === "clinic" ||
      value.function === "culture" ||
      value.function === "recreation" ||
      value.function === "parking" ||
      value.function === "industrial") &&
    Number.isFinite(value.x) &&
    Number.isFinite(value.z) &&
    Number.isFinite(value.rotation) &&
