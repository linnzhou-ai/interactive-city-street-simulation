...
interface ScheduledDemand {
  ...
  retailUnitsByPerson: ReadonlyMap<string, number>;
  paidServiceVisits: ReadonlyMap<string, number>;
  paidServiceWeightByPerson: ReadonlyMap<string, number>;
...
export function rebuildEntityConnections(
  ...
  for (const person of people) {
    for (let index = 1; index < person.schedule.length; index += 1) {
      ...
      const kind: BuildingConnectionKind = current.activity === "work"
        || (current.activity === "home" && previous.activity === "work")
        ? "work"
        : "visit";
      add(kind, previous.buildingId, current.buildingId, person.id);
    }
  }
  ...
  for (const building of buildings) {
    ...
    if (supplier) add("delivery", supplier.id, building.id, undefined, Math.max(1, building.accounting.localSupplies));
    if (building.accounting.importedSupplies > 0) {
      add("delivery", OUTSIDE_MARKET, building.id, undefined, building.accounting.importedSupplies);
    }
  }
...
function advanceDetailedDay(
  ...
  for (const household of households) {
    const rentDue = buildingById.get(household.homeBuildingId)?.rentDaily ?? 0;
    const members = household.memberIds
      .map((id) => people.find((person) => person.id === id))
      .filter((person): person is DetailedPerson => Boolean(person));
    const projectedIncome = sum(members.map((person) => projectedDailyWage(person, buildingById, policy)));
    const availableBeforeAid = Math.max(0, household.money) + projectedIncome;
    const essentialFloor = rentDue + members.length * 10;
    const aidEligible = household.financialStatus === "distressed"
      || household.financialStatus === "crisis"
      || availableBeforeAid < essentialFloor;
    household.assistanceReceived = aidEligible
      ? round(Math.min(45, Math.max(0, essentialFloor - availableBeforeAid) * 0.65 + members.length * 2))
      : 0;
    const rentPaid = round(Math.min(rentDue, availableBeforeAid + household.assistanceReceived));
    rentChargedByHousehold.set(household.id, rentDue);
    rentPaidByHousehold.set(household.id, rentPaid);
    rentCollectedByBuilding.set(
      household.homeBuildingId,
      round((rentCollectedByBuilding.get(household.homeBuildingId) ?? 0) + rentPaid),
    );
  }
  buildings = buildings.map((building) => advanceBuilding(
    ...
    rentCollectedByBuilding.get(building.id) ?? 0,
    city,
    policy,
    day,
    events,
  ...
  for (const person of people) {
    ...
    const perPersonHousing = household
      ? (rentPaidByHousehold.get(household.id) ?? 0) / Math.max(1, household.memberIds.length)
      : 0;
  ...
  for (const household of households) {
    ...
    const rentDue = rentChargedByHousehold.get(household.id) ?? 0;
    const housing = rentPaidByHousehold.get(household.id) ?? 0;
    ...
    const available = Math.max(0, household.money) + income + household.assistanceReceived;
    const unpaidPurchases = Math.max(0, household.dailyExpenses.total - available);
    const rentShortfall = Math.max(0, rentDue - housing);
    let remainingCash = Math.max(0, available - household.dailyExpenses.total);
    let debt = round(household.debt * 1.001 + unpaidPurchases);
    const reserve = members.length * 12;
    let rentArrears = round(household.rentArrears + rentShortfall);
    const arrearsPayment = Math.min(rentArrears, Math.max(0, remainingCash - reserve) * 0.25);
    rentArrears = round(Math.max(0, rentArrears - arrearsPayment));
    remainingCash = Math.max(0, remainingCash - arrearsPayment);
    const debtPayment = Math.min(debt, Math.max(0, remainingCash - reserve) * 0.18);
    debt = round(Math.max(0, debt - debtPayment));
    remainingCash = round(Math.max(0, remainingCash - debtPayment));
    household.money = remainingCash;
    household.debt = debt;
    household.rentArrears = rentArrears;
    const essentialBudget = Math.max(0, available - housing);
    household.unmetEssentials = round(
      rentShortfall + Math.max(0, goods + transport - essentialBudget),
    );
    household.financialStatus = financialStatusFor(household, rentDue);
    const perMemberCash = round(household.money / Math.max(1, members.length));
    for (const person of members) {
      person.money = perMemberCash;
      updateNeedsAndMigration(
        person,
        household,
        updatedBuildingById.get(household.homeBuildingId),
        day,
        events,
        recordEvents,
      );
  ...
  const peopleWithHistory = people.map((person) => ({
    ...
    history: appendHistory(person.history, {
      ...
      housingScore: person.happinessComponents.housing,
      travelScore: person.happinessComponents.travel,
    }),
  }));
  const householdsWithHistory = households.map((household) => ({
    ...
    history: appendHistory(household.history, {
      ...
      debt: household.debt,
      assistanceReceived: household.assistanceReceived,
      unmetEssentials: household.unmetEssentials,
    }),
  }));
  return {
    buildings,
...
function buildingHistoryPoint(
  ...
  const buildingConnections = connections.filter(
    (connection) => connection.fromBuildingId === building.id || connection.toBuildingId === building.id,
  );
  const connectionVolume = (kind: BuildingConnectionKind) => sum(
    buildingConnections.filter((connection) => connection.kind === kind).map((connection) => connection.volume),
  );
  return {
    day,
    ...
    localSupplyCost: accounting.localSupplyCost,
    importedSupplyCost: accounting.importedSupplyCost,
    ...
    goodsReceived: accounting.goodsReceived,
    localSupplies: accounting.localSupplies,
    importedSupplies: accounting.importedSupplies,
    ...
    workLegs: connectionVolume("work"),
    visitLegs: connectionVolume("visit"),
    deliveryUnits: connectionVolume("delivery"),
  };
}
...
function relocateHouseholds(
  ...
  for (const household of households) {
    ...
    const urgent = household.financialStatus === "distressed"
      || household.financialStatus === "crisis"
      || household.rentArrears > 0;
...
function advanceBuilding(
  ...
  const localSupplyCost = round(localSupplies * 5.2);
  const importedSupplyCost = round(importedSupplies * importedUnitCost);
  const supplyCost = round(localSupplyCost + importedSupplyCost);
...
function updateNeedsAndMigration(
  ...
  household: Readonly<DetailedHousehold>,
  home: Readonly<DetailedBuilding> | undefined,
  day: number,
  events: EntityEvent[],
  recordEvents: boolean,
): void {
  ...
  const needsScore = sum(Object.values(person.needs)) / 5;
  const financialBase: Record<HouseholdFinancialStatus, number> = {
    stable: 88,
    strained: 62,
    distressed: 38,
    crisis: 18,
  };
  const debtDays = (household.debt + household.rentArrears) / Math.max(20, household.dailyIncome);
  const financialSecurity = clamp(financialBase[household.financialStatus] - debtDays * 1.4, 5, 96);
  const employmentScore = person.employment === "unemployed" ? 30
    : person.employment === "student" ? 76
      : person.employment === "retired" ? 72
        : 84;
  const rentBurden = home
    ? home.rentDaily / Math.max(25, household.dailyIncome)
    : 1;
  const affordabilityScore = clamp(100 - Math.max(0, rentBurden - 0.2) * 125, 10, 100);
  const housingScore = clamp(
    (home?.accounting.buildingCondition ?? 0.65) * 55 + affordabilityScore * 0.45,
    0,
    100,
  );
  ...
  person.happinessComponents = {
    needs: round(needsScore),
    financialSecurity: round(financialSecurity),
    employment: round(employmentScore),
    housing: round(housingScore),
    travel: round(travelScore),
  };
  person.happiness = round(clamp(
    needsScore * 0.35
      + financialSecurity * 0.25
  ...
  if (person.unemployedDays > 30 || household.financialStatus === "crisis" || person.happiness < 35) {
    person.migrationStatus = person.unemployedDays > 90
      || (household.financialStatus === "crisis" && debtDays > 21)
      ? "moving-out"
      : "considering-leaving";
    person.migrationReason = person.unemployedDays > 30
      ? `${person.unemployedDays} days without work is the strongest reason to leave.`
      : household.financialStatus === "crisis"
        ? `${Math.round(household.rentArrears)} in rent arrears and ${Math.round(household.debt)} in debt are forcing the household to consider leaving.`
    ...
    person.migrationReason = household.financialStatus === "strained"
      ? "The household is cutting discretionary spending, but work and housing remain viable."
      : person.employment === "local"
...
function createSchedule(
  ...
  if ((financialStatus === "distressed" || financialStatus === "crisis") && activity === "leisure") {
    activity = "library";
  }
  ...
  schedule.push(item(
    ...
    workedExternally && !service
      ? externalCommuteMinutes(policy)
      : returnOrigin.id === home.id ? 0 : travelTime(returnOrigin, home, returnMode, policy),
  ));
  return schedule;
}
...
function countScheduledDemand(
  ...
  const retailUnitsByPerson = new Map<string, number>();
  const paidServiceVisits = new Map<string, number>();
  const paidServiceWeightByPerson = new Map<string, number>();
  ...
  for (const person of people) {
    ...
    const paidServiceScale: Record<HouseholdFinancialStatus, number> = {
      stable: 1,
      strained: 0.6,
      distressed: 0,
      crisis: 0,
...
function dailyGoodsCost(
  ...
  return round(
    retailer.accounting.localSalesRevenue
      * (demand.retailUnitsByPerson.get(person.id) ?? 0)
      / totalDemand,
  );
}
...
function dailyServiceCost(
  ...
  const weight = demand.paidServiceWeightByPerson.get(person.id) ?? 0;
  return round(provider && paidVisits > 0 ? provider.accounting.localSalesRevenue * weight / paidVisits : 0);
}

function projectedDailyWage(
  person: Readonly<DetailedPerson>,
  buildings: ReadonlyMap<string, DetailedBuilding>,
  policy: Readonly<EntityPolicy>,
): number {
...
function financialStatusFor(
  ...
  const obligations = household.debt + household.rentArrears;
  const obligationDays = obligations / Math.max(20, household.dailyIncome);
  if (household.rentArrears > rentDue * 7 || obligationDays > 21 || household.unmetEssentials > 35) {
    return "crisis";
  }
  if (household.rentArrears > rentDue * 2 || obligationDays > 7 || household.unmetEssentials > 12) {
    return "distressed";
  }
  if (obligations > 0 || (
    household.dailyExpenses.total > household.dailyIncome * 1.08
    && household.money < household.dailyExpenses.total * 30
  )) {
    return "strained";
  }
  return "stable";
