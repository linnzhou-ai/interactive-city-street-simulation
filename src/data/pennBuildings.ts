...
function createBlockBuildings(
  ...
  for (let avenueIndex = 0; avenueIndex < PENN_AVENUES.length - 1; avenueIndex += 1) {
    for (let streetIndex = 0; streetIndex < PENN_STREETS.length - 1; streetIndex += 1) {
      ...
      const blockLandmarks = landmarks.filter(
        (landmark) =>
      ...
      for (let index = 0; index < count; index += 1) {
        ...
        const candidate: EntityBuildingDefinition = {
          ...
          address: `${
            3000 + (fillsLandmarkGap ? infillNumber : buildingNumber) * 7
          } ${street.name}`,
          source: "block",
...
function createLandmarkBuildings(): EntityBuildingDefinition[] {
  return PENN_LANDMARKS.map((landmark, index) => {
    const [requestedWidth, requestedDepth, height] =
      LANDMARK_SIZES[landmark.kind];
...
function footprintsOverlap(
  ...
  return (
    Math.abs(a.x - b.x) <
      aFootprint.halfX + bFootprint.halfX + clearance &&
    Math.abs(a.z - b.z) <
      aFootprint.halfZ + bFootprint.halfZ + clearance
  );
}
...
function fitAroundLandmarks(
  ...
  const gap =
    gaps[1].max - gaps[1].min > gaps[0].max - gaps[0].min
      ? gaps[1]
      : gaps[0];
